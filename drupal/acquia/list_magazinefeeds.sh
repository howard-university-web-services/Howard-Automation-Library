#!/bin/bash
#
# List non Magazine Feed Widget use on on all howard D8 Sites.
#
...
echo "Listing Magazine Feeds for Howard sites."
...
for APP in ${LOCAL_HOWARD_D8_DRUSH_ALIAS[@]}; do
  echo "Running magazine feed lists for $APP$ENV"
  ${LOCAL_DRUSH} $APP$ENV ssh "bash /var/www/html/"\${AH_SITE_NAME}"/scripts/hal_magazinefeed_list.sh"
done
